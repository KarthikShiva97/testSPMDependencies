...
/** \ingroup enum_defs
 *
 * YOSessionState defines the state for the Yospace Session.
 */
typedef NS_ENUM(NSInteger, YOSessionState) {
    /** The default state of the session. */
    YONone,
...
/** \ingroup const_defs
 *
 * Yospace initialisation result code: The CSM delivered VMAP that contained placeholder ad breaks. */
#define PARTIAL_VMAP -22
/** \ingroup const_defs
 *
...
typedef NS_ENUM(NSInteger, YOPlayerEvent) {
    /** Indicates that playback is ready */
    YOPlaybackReadyEvent,
 ...
 */
typedef NS_OPTIONS(NSInteger, YODebugFlags) {
 ...
 */
typedef NS_OPTIONS(NSInteger, YOEventCategories) {
...
/** \ingroup enum_defs
 * 
 * YOSessionError defines the set of error codes that are associated with a Session error notification.
 * @see YOSessionErrorNotification
 */
typedef NS_ENUM(NSInteger, YOSessionError) {
    /** The session with the Yospace Central Streaming Manager (CSM) service timed-out. No further analytics will be signalled after this event. */
    YOSessionTimeout,
    /** The fulfilment payload for an initial partial VMAP response was not received or did not contain the expected data. */
    YOUnresolvedBreak
};

/** \ingroup enum_defs
 *
 * YOPlaybackControlEvent defines the set of playback control events used for interactions with the player.
 */
typedef NS_ENUM(NSInteger, YOPlaybackControlEvent) {
    YOPlaybackControlAnalyticUpdateEvent
};
